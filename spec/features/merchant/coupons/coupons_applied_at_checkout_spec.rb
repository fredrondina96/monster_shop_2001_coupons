...
RSpec.describe 'When I view my cart as a user' do
  describe 'Any merchant coupons are automatically applied' do
    before(:each) do
      ...
      it "only items from the same merchant as the coupon are discounted" do
        visit "/items/#{@pencil.id}"
        click_on "Add To Cart"

        visit "/cart"
          within "#item-quantity-#{@pencil.id}" do
            4.times do
              click_link "+"
            end
          end
      ...
      it "if there are two coupons that can be applied, the one with the greater discount is applied" do

        @coupon2 = @meg.coupons.create!(name: "90% off 8 Items", percent_off: 90, quantity_required: 4)
        ...
        expect(page).to_not have_content("$640.00")
